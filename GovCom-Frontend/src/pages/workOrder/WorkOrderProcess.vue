    ...
    <a-card title="处理工单">
      ...
      <div v-else-if="order">
        <div class="order-info">
          <h3>工单信息</h3>
          <p><strong>工单编号：</strong>{{ order.orderNo }}</p>
          <p><strong>工单类型：</strong>{{ order.typeName }}</p>
          <p><strong>申请人：</strong>{{ order.applicantName }}</p>
          <p><strong>联系电话：</strong>{{ order.applicantPhone }}</p>
          <p><strong>申请内容：</strong>{{ order.content }}</p>
        </div>
        ...
        <a-form
          ...
          layout="vertical"
        >
          <a-form-item label="处理结果" name="result" required>
            <a-textarea
              v-model:value="form.result"
              placeholder="请输入处理结果或驳回理由"
              :rows="4"
            />
          </a-form-item>
...
import { getWorkOrderDetailUsingGet, processWorkOrderUsingPost } from '@/api/workOrderController'
...
const handleSubmit = async () => {
  ...
  try {
    const res = await processWorkOrderUsingPost({
      id: order.value.id,
