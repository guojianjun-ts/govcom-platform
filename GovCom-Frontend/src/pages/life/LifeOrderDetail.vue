<!-- src/pages/life/LifeOrderDetail.vue -->
<template>
      ...
      <a-card class="detail-card">
        ...
        <div class="info-section">
          ...
          <a-descriptions :column="1">
            <a-descriptions-item label="申请单号">{{ order.orderNo }}</a-descriptions-item>
            <a-descriptions-item label="申请类型">{{ order.typeName }}</a-descriptions-item>
          ...
          <div class="result-box">
            ...
            <div v-if="order.resultFiles && order.resultFiles.length > 0" class="result-files">
              <div v-for="(file, index) in order.resultFiles" :key="index">
                <a :href="file" target="_blank">📎 证明文件{{ index + 1 }}</a>
              </div>
            </div>
          </div>
        </div>
...
import { getOrderDetailUsingGet } from '@/api/lifeController'
...
const fetchDetail = async (id: number) => {
  loading.value = true
  try {
    const res = await getOrderDetailUsingGet({ id })
    if (res.code === 0) {
